...
if (
  window.location.pathname === '/cuisines/mexicanCuisine.html' ||
  window.location.pathname === '/dummy.html'
) {
  document.addEventListener('DOMContentLoaded', function () {
    var cuisine = window.location.pathname
      .split('/cuisines/')[1]
      .split('Cuisine.html')[0]
      .toLowerCase();
    console.log(cuisine);
    getRecipe(cuisine);
...
function createRecipeCard(title, image, ingredients, instructions, cuisine) {
  ...
  existingRecipeCards.forEach(function (cardTitle) {
    if (cardTitle.innerText.trim() === title.split('+').join(' ')) {
      ...
      var existingFaveIcon = cardTitle
        .closest('.card')
        .querySelector('.fa-heart');
      if (
        existingFaveIcon &&
        !existingFaveIcon.classList.contains('fa-solid')
      ) {
  ...
  if (duplicateFound) {
    console.log(
      'Duplicate recipe found. Removing the second occurrence:',
      title
    );
    return;
  }
  ...
  recipeContainer.setAttribute(
    'style',
    'justify-content: flex-start; margin: 41px;'
  );
  var recipeCard = document.createElement('div');
  recipeCard.setAttribute(
    'class',
    'card col-sm-6 col-md-4 p-3 text-center justify-content-between mb-4'
  );
  recipeCard.setAttribute(
    'style',
    'width: 20.5rem; height: 350px; background-color: #fdefdc; border: 1px solid #fdefdc; color: #404146; margin: auto;'
  );
  ...
  recipeBtnBody.setAttribute(
    'style',
    'display: flex; justify-content: space-around;'
  );
  var recipeDetailsBtn = document.createElement('button');
  var recipeFave = document.createElement('i');
  recipeFave.setAttribute(
    'class',
    isFavourited(title) ? 'fa-solid fa-heart' : 'fa-regular fa-heart'
  );
  recipeFave.setAttribute(
    'class',
    'fa-regular fa-heart d-flex justify-content-center align-items-center'
  );
  ...
  var heartId = document.querySelector(
    `[data-target="${title.split('+').join(' ')}"]`
  ).dataset.target;
  console.log(heartId);
  ...
  var modalHTML = `
        <div class="modal fade" id="${title
          .split(' ')
          .join('')}" tabindex="-1" aria-labelledby="${title
    .split(' ')
    .join('')}label" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                <div class="modal-content" id="Modal-${title
                  .split('+')
                  .join('')}">
                    <div class="modal-header">
                        <h1 class="modal-title fs-3" style="width: 90%;" id="${title
                          .split('+')
                          .join('')}label">${title.split('+').join(' ')}</h1>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <h2>Nutritional Information<sup>*</sup></h2>
                        <div id="nutritionInfo${title
                          .split('+')
                          .join('')}"></div><br>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn bg-gold text-white" id="fave${title
                          .split('+')
                          .join('')}">${modalBtn}</button>
  ...
  var faveRecipeEl = document.getElementById(
    `fave${title.split('+').join('')}`
  );
...
function updateNutritionInfo(title, nutritionData) {
  var nutritionInfoDiv = document.getElementById(
    `nutritionInfo${title.split('+').join('')}`
  );
...
function showAllFavourites(currentPageCuisine) {
  ...
  for (var i = 0; i < localStorage.length; i++) {
    ...
    if (isRecipe(storedValue)) {
      ...
      var shouldShowRecipe =
        !currentPageCuisine ||
        (recipeDetails.cuisine &&
          recipeDetails.cuisine.toLowerCase() ===
            currentPageCuisine.toLowerCase());
      ...
      if (window.location.pathname === '/favourites.html') {
        var heart = document.querySelector(
          `[data-target="${recipeDetails.title}"]`
        );
        if (heart) {
        ...
        var faveRecipeEl = document.getElementById(
          `fave${recipeDetails.title}`
        );
        console.log(faveRecipeEl);
        faveRecipeEl.setAttribute(
          'id',
          `fave${recipeDetails.title.split(' ').join('')}`
        );
